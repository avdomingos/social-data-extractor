...
@CommandAnnotation(command = "executeDistributedStratex",
        description = "Execute the stratex application with the specified parameters",
        usage = "executeDistributedStratex [-follow <all|userID1;userID2;userID3;..;userIDN>|-track <track1;track2;track3;..;trackN>] -numberOfInstances <all|numberOfInstances>")
public class CommandExecuteDistributedStratex extends Command {
    ...
    protected void commandAction(PrintStream ps, Map<String, String> parameters) throws ExitWithoutSuccessException {
        try {
            if (parameters.containsKey("follow") && parameters.containsKey("track")) {
                DistributedMonitorization.queueWork(DistributedMonitorization.WorkType.BOTH, parameters);
            } else if (parameters.containsKey("track")) {
                DistributedMonitorization.queueWork(DistributedMonitorization.WorkType.TRACK, parameters);
            } else if (parameters.containsKey("follow")) {
                DistributedMonitorization.queueWork(DistributedMonitorization.WorkType.FOLLOW, parameters);
            } else {
